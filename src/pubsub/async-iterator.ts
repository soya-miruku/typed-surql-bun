...
export class PubSubAsyncIterator<T> implements AsyncIterator<T> {
  ...
  public async return(): Promise<IteratorResult<T>> {
    ...
    this.pubsub.emit("ended", this.eventsArray)
  ...
  private async pushValue(event: T) {
    ...
    if (this.pullQueue.length !== 0) {
      this.pullQueue.shift()?.(this.running ? { value: event, done: false } : { value: undefined, done: true });
    } else {
    ...
    return new Promise(
      resolve => {
        if (this.pushQueue.length !== 0) {
          //@ts-ignore
          resolve(this.running ? { value: this.pushQueue.shift(), done: false } : { value: undefined, done: true },
          );
        } else {
